...
| `catalog-character-model-selection` | 모든 catalog provider의 캐릭터→모델·에디션 선택, 계층 정규화, 기본 Pet 이름과 preset 복원 |
| `provider-repository-boundary` | Provider 권리물 비포함, local 운영 도구·실자산 smoke 격리와 전역 repository merge gate |
