...
웹 앱은 same-origin `localStorage`의 전용 versioned document에 Pet 설정 preset을 저장해야 한다(MUST). preset key는 trim된 Pet 표시 이름이어야 하며(MUST), 각 preset은 Pet 이름·설명, framing scale·X/Y offset, 눈 이동량, 전체 수평 반전과 9개 상태의 animation 이름·상태별 반전만 포함해야 한다(MUST). model byte, ZIP, atlas/PNG, source/provider URL, character ID, download URL, 생성 package와 validation 결과를 저장해서는 안 된다(MUST NOT). 저장소는 최대 20개 preset으로 제한해야 한다(MUST).
...
- **WHEN** 사용자가 `Miku` preset을 저장한 뒤 page를 다시 열고 source를 수동으로 불러온다
- **THEN** selector는 최초부터 `Miku`를 선택해야 한다
- **AND** source가 ready가 된 뒤 저장된 metadata, framing, 눈 이동량, 반전과 유효 mapping이 적용되어야 한다

#### Scenario: 자동 source load 금지
