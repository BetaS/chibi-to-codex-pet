...
사용자-facing source 우선순위는 다음과 같다.

```text
1. 승인 manifest의 panxuc commit-pinned jsDelivr snapshot (기본)
2. 사용자가 이미 가진 canonical ZIP (고급 fallback)
...
Snapshot의 buildData와 `_info.json`은 `s000_templete_MyGO`와 `s000_templete_mygo`처럼 같은 model identity의 대소문자가 다를 수 있다. Resolver는 ASCII case-fold 후 정확히 하나인 catalog key·file만 실제 snapshot entry로 사용하고, 둘 이상이 같아지는 충돌은 임의 선택 없이 거부한다. Canonical `modelName`은 buildData가 선언한 원래 표기를 보존한다.
...
Preview의 CSS layout과 canonical projection은 `192:208` 비율을 유지하되, canvas backing store와 WebGL viewport는 현재 display pixel ratio만큼 확대한다. 이 고밀도 preview 배율은 화면 표시 품질에만 적용하고 결정적인 `192×208` export cell과 framing 수치에는 적용하지 않는다.
...
### 10. Live resource pack을 기본으로 하고 model 선택을 즉시 적용한다

Garupa panel은 승인된 pinned live resource pack을 기본 경로로 표시한다. Catalog 요청은 기존처럼 사용자의 명시적인 목록 action 뒤에만 시작하지만, 목록이 준비되면 UI는 bundle 690개를 한 목록으로 노출하지 않고 locale별 고유 캐릭터를 먼저 고른 다음 그 캐릭터에 매핑된 model bundle을 고르게 한다. Character metadata에 안전하게 매핑되지 않은 bundle은 임의 캐릭터에 귀속하지 않고 별도 미매핑 group에서 접근할 수 있게 한다.

캐릭터 선택은 model request를 시작하지 않는다. 두 번째 model combobox 선택은 controller selection을 먼저 원자적으로 갱신하고 별도 `불러오기` 버튼 없이 해당 generation의 materialization과 preview를 즉시 시작한다. 빠른 재선택은 이전 generation을 abort하고 마지막 model만 active source가 될 수 있다. Local canonical ZIP은 접힌 고급 기능에 두고 파일 선택 뒤의 명시적 local load를 유지한다.

Preview session은 전달받은 canvas의 WebGL context를 소유하지 않는다. 각 session은 자신이 만든 runtime session, texture, shader, atlas와 animation frame만 dispose하고 `WEBGL_lose_context`를 호출하거나 canvas 크기를 1×1로 축소해서는 안 된다. 같은 canvas에서 새 preview의 첫 visible frame이 준비된 뒤 이전 session을 dispose해도 새 preview context가 유지되어야 하며, 새 preview 생성 실패 시에도 기존 ready preview가 다음 frame을 계속 그릴 수 있어야 한다. Canvas context의 최종 수명은 React integration과 DOM canvas 제거가 소유한다.
