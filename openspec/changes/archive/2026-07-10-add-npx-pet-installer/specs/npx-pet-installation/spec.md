...
시스템은 Node.js `>=22.12.0`에서 실행되는 local `@chibi-to-codex-pet/cli` package와 `chibi-to-codex-pet` bin을 제공해야 한다(MUST). 첫 공개 설치 명령은 repo root에서 `npm_config_cache=.npm-cache npx --yes --package ./packages/cli -- chibi-to-codex-pet install --recipe <recipe>`를 지원해야 하며(MUST), `<recipe>`는 base64url JSON, raw JSON 또는 HTTPS recipe URL로 해석할 수 있어야 한다(MUST). CLI는 `--help`, `--version`, `--codex-home`, `--dry-run`, `--force`를 비대화형으로 처리해야 한다(MUST).

#### Scenario: npx recipe 설치 명령 실행
- **WHEN** 사용자가 repo root에서 `npm_config_cache=.npm-cache npx --yes --package ./packages/cli -- chibi-to-codex-pet install --recipe <recipe>`를 실행한다
...
Recipe는 `schemaVersion: 1`, `kind: "livesd-recipe"`, `renderer: "livesd36-codex-pet@1"`, remote `source`, `pet` metadata/framing scale, 그리고 모든 표준 Codex Pet 상태의 animation mapping을 포함해야 한다(MUST). Recipe는 PNG, ZIP, base64 spritesheet 또는 local file path를 포함해서는 안 된다(MUST NOT).

#### Scenario: 유효한 prsk-chibi-viewer recipe
- **WHEN** recipe source가 `provider: "prsk-chibi-viewer"`와 safe `characterId`를 가진다
...
- **WHEN** recipe source가 `provider: "custom"`, canonical HTTPS `assetBaseUrl`, safe `characterId`를 가진다
...
CLI는 recipe를 package-internal headless browser renderer에 전달해야 하며(MUST), renderer는 기존 remote resource loader, LiveSD frame sampler, Codex Pet exporter와 package validator를 재사용해야 한다(MUST). Renderer가 반환한 payload는 설치 전에 `pet.json`과 `spritesheet.png` 두 파일이어야 하며(MUST), manifest ID와 spritesheet v2 계약을 통과해야 한다(MUST).

#### Scenario: recipe를 v2 Pet payload로 렌더링
- **WHEN** recipe source와 mappings가 유효하고 remote resources가 기존 browser validator를 통과한다
