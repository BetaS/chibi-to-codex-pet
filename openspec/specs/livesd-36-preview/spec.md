...
#### Scenario: Remote 공통 스켈레톤 선택
- **WHEN** provided 또는 custom source 사용자가 유효한 catalog dropdown에서 캐릭터와 모델을 명시적으로 선택한다
- **THEN** 시스템은 검증·제한된 원격 스켈레톤 `ArrayBuffer`를 버전 검사 단계에 전달한다

#### Scenario: Upload 공통 스켈레톤 누락
- **WHEN** upload source에서 사용자가 공통 스켈레톤 파일을 선택하지 않는다
...
시스템은 runtime 독립 `LiveSDAtlasBundle`의 atlas 페이지로 texture를 생성하고 `TextureAtlas`, `AtlasAttachmentLoader`, `SkeletonBinary`, `Skeleton`, `AnimationStateData`, `AnimationState`를 구성해 alpha가 활성화되고 depth와 stencil이 비활성화된 WebGL canvas에 렌더링해야 한다(MUST). 상태 매핑이 준비되면 미리보기는 export sampler와 같은 57개 표준 pose, sample time, `192×208` calibration viewport, raw bounds 합집합의 축별 10% padding, 실제 alpha bounds의 1px guard 합집합과 8px final inset으로 canonical visible bounds와 projection을 계산해야 한다(MUST). 이 projection에 동일한 `framingScale`, `framingOffset`과 수평 반전을 적용해 현재 Spine animation pose를 실시간으로 렌더링해야 한다(MUST). Texture가 투명한 attachment를 포함하는 raw `Skeleton.getBounds()`를 최종 framing 근거로 사용해서는 안 된다(MUST NOT). 상태 매핑이 아직 없는 adapter bootstrap 단계에서는 현재 animation 첫 pose의 실제 alpha bounds를 이용한 임시 projection을 사용할 수 있지만(MAY), 상태 매핑을 받은 뒤에는 이를 canonical projection으로 교체해야 한다(MUST). Canvas backing buffer와 viewport는 CSS 크기에 `max(devicePixelRatio, 1)`을 곱한 값을 내림한 최소 1px 크기로 맞춰야 한다(MUST). 매 frame draw 전에 depth test와 depth write 및 cull, scissor, stencil, polygon offset state를 비활성화하고(MUST), attachment를 임의 z 값이나 이름으로 재정렬하지 않은 채 현재 `skeleton.drawOrder`와 alpha blending으로 겹침 순서를 결정해야 한다(MUST). 세션은 첫 frame draw까지 성공한 뒤에만 ready가 되어야 한다(MUST).

#### Scenario: 미리보기 생성 성공
- **WHEN** 공통 스켈레톤과 유효한 `LiveSDAtlasBundle` 및 WebGL canvas가 주어지고 첫 frame draw가 성공한다
- **THEN** 시스템은 투명 배경에 첫 pose를 그린 ready 세션을 반환한다
- **AND** 상태 매핑을 받으면 57개 표준 pose로 계산한 canonical projection으로 현재 pose를 다시 그린다

#### Scenario: Preview와 export의 자동 fit 경계
- **WHEN** 같은 source, 상태 매핑, scale, offset과 mirror로 동일한 pose를 LiveSD preview와 package cell에 렌더링한다
- **THEN** 두 경로는 같은 57개 표준 pose에서 얻은 canonical visible bounds와 final projection을 사용해야 한다
- **AND** pose의 pixel-space 크기, 위치와 crop은 WebGL raster 반올림 오차 안에서 일치해야 한다
- **AND** preview의 wall-clock animation 재생과 export의 명시적 sample time 선택만 서로 독립적이어야 한다
...
- **THEN** 시스템은 상태 매핑에서 계산한 canonical projection을 유지한 채 새 animation pose를 그린다
- **AND** animation 전환만으로 alpha calibration이나 bounds 측정을 다시 수행하지 않는다
...
- **THEN** renderer는 draw 전에 depth test와 depth write 및 cull·scissor·stencil·polygon offset을 다시 비활성화해 같은 입력을 같은 순서로 렌더링한다

#### Scenario: 높은 DPR canvas
- **WHEN** CSS preview가 `192×208`이고 `devicePixelRatio`가 3이다
- **THEN** WebGL backing buffer와 viewport는 `576×624`가 되고 CSS border box의 `192:208` geometry는 유지되어야 한다
...
ready WebGL preview는 최종 `192×208` cell과 같은 aspect ratio의 visible border box를 표시해야 하며(MUST), 아직 ready 캐릭터가 없으면 이 border box와 canvas를 숨기고 빈 상태 안내만 표시해야 한다(MUST). 자동 안전 fit 대비 `80%–150%`, 기본 `100%`, 1% 단위의 Pet 크기와 최종 cell pixel 기준 X `-96..96`, Y `-104..104`, 기본 `0,0`, 1px 단위 offset control을 제공해야 한다(MUST). `+X`는 오른쪽, `+Y`는 아래쪽 이동이어야 한다(MUST). 각 control은 보이는 label, 현재 값과 단위, keyboard 조작과 접근 가능한 현재값을 제공해야 하며(MUST), preview가 없을 때 비활성화되어야 한다(MUST). Framing 변경은 cached canonical visible bounds에 export와 같은 final projection 계산으로 즉시 적용하고(MUST), 현재 animation pose를 다시 그려야 한다(MUST). 선택 framing과 canonical bounds는 animation 변경과 canvas resize에서 유지하고(MUST), 상태 매핑의 animation 이름이 바뀌면 57개 표준 pose calibration을 다시 수행해야 한다(MUST). 상태별 mirror만 바뀐 경우에는 canonical bounds를 다시 측정해서는 안 된다(MUST NOT). 새 source가 성공적으로 ready가 되면 framing은 `100%, 0, 0`으로 초기화해야 한다(MUST).

#### Scenario: 캐릭터 없는 preview stage
- **WHEN** page 진입 직후이거나 source가 아직 ready가 아니어서 표시할 캐릭터가 없다
- **THEN** LiveSD border box와 canvas는 보이지 않아야 한다
- **AND** 캐릭터가 없다는 빈 상태 안내는 보여야 한다

폭이 `58.001rem` 이상인 desktop에서는 control panel과 preview/builder를 두 열로 배치하고 preview panel 너비를 `min(100%, 48rem)`, 높이를 `clamp(33rem, 58vh, 36rem)`으로 제한해 두 번째 열의 가로 중앙에 배치해야 한다(MUST). LiveSD border box는 stage의 content box 안에서 `192:208` 비율을 유지하는 contain 크기로 계산해야 하고(MUST), border box의 전체 border rect가 stage의 가로·세로 경계를 넘어서 `overflow: hidden`으로 잘려서는 안 된다(MUST NOT). 폭이 `58rem` 이하면 control, preview, builder를 한 열 순서로 배치하고 preview panel의 최소 높이는 36rem, border box의 너비는 `min(100%, 22rem)`이어야 한다(MUST). 폭이 `36rem` 이하면 preview toolbar control을 한 열로 쌓아 viewport 가로 overflow를 만들지 않아야 한다(MUST).
...
- **THEN** session은 세 값과 canonical visible bounds를 유지하고 현재 pose에 같은 final projection을 적용한다

#### Scenario: 상태 매핑 변경 시 canonical bounds 갱신
- **WHEN** 하나 이상의 상태에 선택된 animation 이름을 변경한다
- **THEN** session은 변경된 57개 표준 pose plan으로 canonical visible bounds를 다시 계산한다
- **AND** scale, offset과 현재 animation은 유지한다
...
#### Scenario: Desktop preview 크기 제한
- **WHEN** viewport 폭이 58.001rem 이상이다
- **THEN** preview panel은 48rem보다 넓어지지 않고 두 번째 workspace 열의 가로 중앙에 배치된다
- **AND** panel은 33rem보다 작거나 36rem보다 커지지 않고 viewport 높이의 58%를 선호해 출력 프레임의 contain 공간을 확보해야 한다
- **AND** `192:208` border box의 전체 rect는 stage content box 안에 들어와 어느 변도 잘리지 않아야 한다
- **AND** builder는 같은 workspace의 preview 아래에 이어져 별도 full-screen preview layout을 만들지 않아야 한다
...
#### Scenario: 선택 animation으로 popup 자동 스크롤
- **WHEN** animation combobox를 열고 현재 선택값이 긴 option 목록의 중간 또는 끝에 있다
- **THEN** 시스템은 선택 option을 초기 highlight로 삼고 `scrollIntoView({ block: "nearest" })`와 동등한 동작으로 listbox viewport 안에 표시해야 한다
...
`LiveSDPreviewSession`은 현재 animation pose, canonical visible bounds, framing scale과 canvas 크기를 유지하면서 전체 렌더를 bounds 중심 기준으로 수평 반전할 수 있어야 한다(MUST). 반전은 projection에서 수행하고(MUST), skeleton bone·attachment geometry·draw order를 변경해서는 안 된다(MUST NOT). 반전 변경은 새 session, animation restart, canonical bounds 재측정 또는 network request 없이 현재 pose에 즉시 다시 그려져야 한다(MUST).
...
#### Scenario: 반전 중 animation 전환
- **WHEN** 반전된 session에서 다른 animation을 선택한다
- **THEN** 시스템은 기존 canonical visible bounds와 반전 final projection을 유지한다
- **AND** 새 animation pose만 현재 반전 projection으로 렌더링한다
