# PRSK 캐릭터 아카이브 가져오기 명세

## Purpose

사용자가 선택한 PRSK character ZIP을 browser memory에서 검증하고 runtime 독립 `LiveSDAtlasBundle`로 정규화한다. ZIP은 character atlas와 texture page를 제공하며 공통 skeleton은 별도 입력으로 관리한다.

## Requirements

### Requirement: PRSK 캐릭터 ZIP 입력 계약
시스템은 upload source에서 사용자가 선택한 단일 PRSK character ZIP을 browser로 가져와야 한다(MUST). 유효한 ZIP은 정확히 하나의 `sekai_atlas.atlas`, 해당 atlas가 참조하는 모든 PNG page와 암호화되지 않은 entry로 구성되어야 하며(MUST), skeleton은 별도 `.skel` 입력으로 받아야 한다(MUST). Atlas와 참조 PNG 이외의 file은 결과 bundle에서 제외하되 모든 archive 검증과 resource limit에는 포함해야 한다(MUST).
...
`PrskCharacterArchiveImporter`는 upload ZIP의 source 이름, atlas 경로와 원문 및 페이지별 Blob map을 포함한 runtime 독립 `LiveSDAtlasBundle`을 반환해야 한다(MUST). ZIP 검사와 압축 해제는 browser memory에서 수행해야 한다(MUST).
...
#### Scenario: Upload source의 browser-local 처리
- **WHEN** upload source에서 캐릭터 ZIP을 검사하고 압축 해제한다
- **THEN** importer는 File API와 browser memory만 사용하고 network request를 생성하지 않는다
