# Garupa pinned snapshot source 명세

## Purpose

고정된 bangdream-live2d snapshot에서 catalog를 만들고 무결성을 검증한 자산만 안전하게 canonical Garupa source로 materialize하는 계약을 정의한다.

## Requirements

### Requirement: 고정된 bangdream-live2d provider manifest

시스템은 `src/features/livesd/garupa/remote/` 아래의 하나의 frozen typed manifest로 사용자에게 제공할 `bangdream-live2d` snapshot을 정의해야 한다(MUST). Manifest는 `schemaVersion: 1`, stable provider ID, `gameId: "garupa"`, `assetFamily: "sdchara"`, repository URL, 정보용 branch 이름, lowercase 40자리 commit `sourceRevision`, 해당 commit URL, delivery 종류와 base URL, asset index·character metadata path, source region semantics, 예상 runtime profile과 catalog SHA-256·byte length를 포함해야 한다(MUST). Provider 라이선스 검토 결과는 runtime manifest field, source availability gate 또는 사용자-facing UI 정보로 사용해서는 안 된다(MUST NOT).

#### Scenario: 고정 provider manifest
- **WHEN** bundled Garupa provider manifest를 검증한다
- **THEN** repository는 `panxuc/bangdream-live2d`, asset family는 `sdchara`, revision은 정확한 full commit SHA이고 delivery base는 같은 SHA를 포함한 URL이다
...
#### Scenario: Provider 라이선스 정보 비노출
- **WHEN** 사용자가 pinned Garupa source 설명을 보거나 public runtime manifest를 검사한다
- **THEN** UI와 runtime manifest에는 provider 라이선스 상태나 프로젝트의 법적 판단이 포함되지 않아야 한다
- **AND** source 가용성은 provider 라이선스 검토 여부와 무관하게 기능·무결성 검증과 game registry가 결정해야 한다

### Requirement: Pinned delivery URL과 요청 정책

Provider resolver는 manifest가 고정한 HTTPS origin, repository와 full commit base 아래의 안전한 상대 path만 URL로 만들어야 한다(MUST). Request는 CORS mode, credentials omit, no-referrer, redirect error, timeout·stream byte limit과 명시적 `AbortSignal`을 사용해야 하며(MUST), jsDelivr transport에서는 압축 가능한 파일에 Range header를 보내서는 안 된다(MUST NOT). Manifest에 없는 origin, query, fragment, userinfo, absolute child URL, backslash, 빈·점 segment와 path traversal은 request 전에 거부해야 한다(MUST).

#### Scenario: 고정된 commit URL
- **WHEN** resolver가 `sdchara/_info.json` 또는 선택된 model asset path를 받는다
- **THEN** URL은 manifest와 같은 full commit의 고정 base 아래에서 생성되고 request는 credentials 없이 전체 body를 읽는다
...
- **WHEN** pinned snapshot에서 `00001` buildData를 선택한다
...
- **WHEN** 사용자가 pinned source의 검증된 catalog에서 캐릭터를 선택한 뒤 model을 선택한다
- **THEN** 해당 generation만 manifest가 허용한 origin에 request하고 성공 결과를 canonical importer와 runtime에 전달한다

#### Scenario: 저장 pinned model의 명시적 preset 복원
- **WHEN** 저장 preset의 source가 안전한 `sdAssetBundleName`을 가진 `garupa-pinned`이고 사용자가 `프리셋 불러오기`를 실행한다
