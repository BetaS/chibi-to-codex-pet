...
| 영역 | 지원 내용 |
|---|---|
| 정식 게임 integration | 프로세카 (`prsk`), BanG Dream! (`garupa`) |
| 준비 중인 integration | 레뷰 스타라이트 (`strr`) |
...
`garupa`는 이 feature branch에서 선택 가능한 integration으로 제공되며 local canonical ZIP 또는 승인된 pinned snapshot을 명시적으로 불러와 Spine 4.0 preview와 Codex Pet package를 만들 수 있습니다. `strr`는 계속 비활성 준비중 탭입니다.
...
Garupa Spine SD 조사는 `bangdream-live2d`의 승인된 full commit만 사용합니다. 사용자-facing metadata manifest는 개발 서버와 build에서 deployment base 아래의 `manifests/garupa/bangdream-live2d.v1.json`으로 제공되며 repository, commit, catalog hash, 요청 정책과 미선언 data license 상태만 포함합니다. 원본 모델 byte는 포함하지 않습니다.
...
승인된 private fixture와 공식 Spine WebGL `4.0.31` production dependency로 skeleton parse와 첫 WebGL frame을 검증하려면 다음 명령을 사용합니다. `GARUPA_PROBE_FULL=1`은 실제 9-state mapping, straight-alpha, paired-eye 16방향과 결정적 73-frame atlas를 메모리에서 두 번 생성해 독립 decode/cell validator까지 실행합니다. Family command는 private snapshot의 15개 model family를 parse하고 paired-eye가 없는 cat family를 preview-only로 분류합니다.
...
GA4 분석은 선택 사항이며 measurement ID가 없으면 Google script를 요청하지 않습니다. 운영자가 방문과 Pet ZIP 다운로드 관심도를 개인 GA4 dashboard에서 확인하려면 다음과 같이 설정합니다.
...
3. `main`을 다시 배포한 뒤 GA4 Realtime 또는 DebugView에서 방문과 `pet_zip_download` event를 확인합니다.

Local build에서만 확인하려면 `.env.local`의 `VITE_GA_MEASUREMENT_ID`에 같은 값을 설정할 수 있습니다. `gtag('config', ...)`가 최초 `page_view`를 측정하고, `pet_zip_download`는 검증된 ZIP의 다운로드 링크를 누른 경우에만 전송됩니다. 이 event는 실제 OS 저장 완료가 아니라 클릭 횟수를 뜻하며 Pet 이름·설명·파일명, source URL·캐릭터 ID와 animation mapping을 포함하지 않습니다. 앱에는 방문자 수나 다운로드 수를 공개 표시하지 않으며 reporting credential도 포함하지 않습니다.
