...
- 브라우저 `localStorage` 기반 설정 preset
...
생성된 `dist/` 전체를 HTTPS와 SPA fallback을 지원하는 정적 host에 배포합니다. 배포 산출물은 애플리케이션, vendored runtime과 필수 license 고지로 구성됩니다. 모델 asset은 사용자 또는 provider가 runtime에 공급하고 개인 `.env`는 local·deployment 환경에서 별도로 관리합니다.

Codex Sites를 사용할 때는 예시 설정을 개인 설정으로 복사하고 자신의 project ID를 입력합니다.
